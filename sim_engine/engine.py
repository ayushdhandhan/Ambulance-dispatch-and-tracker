...
class Simulation:
    ...
    def load_state(self):
        try:
            self.ambulances = requests.get(f"{API_URL}/ambulances").json()
            self.signals = requests.get(f"{API_URL}/signals").json()
            print(f"Loaded {len(self.ambulances)} ambulances and {len(self.signals)} signals")
        except Exception as e:
            print("Error loading state:", e)

    def dispatch_ambulance(self, ambulance_id, target_lat, target_lng):
        # find ambulance
        amb = next((a for a in self.ambulances if a['id'] == ambulance_id), None)
        if not amb: return
        
        # Call OSRM
        osrm_url = f"https://router.project-osrm.org/route/v1/driving/{amb['lng']},{amb['lat']};{target_lng},{target_lat}?overview=full"
        res = requests.get(osrm_url).json()
        if res.get('code') == 'Ok':
            route_poly = res['routes'][0]['geometry']
        ...
        for amb in self.ambulances:
            if amb.get('status') == 'EN_ROUTE' and 'route' in amb:
                ...
                if idx < len(amb['route']):
                    ...
                    # Ambulance reached destination, set it back to AVAILABLE after a delay
...
def main():
    ...
    for _ in range(10):
        try:
            requests.get(API_URL + "/ambulances")
            break
        except:
            time.sleep(1)
    ...
    def on_manual_dispatch(data):
        ...
        sim.dispatch_ambulance(data['ambulance_id'], data['lat'], data['lng'])
